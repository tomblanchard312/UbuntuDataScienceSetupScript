...
usage() {
	cat <<'EOF'
Usage: ./hyper_v_post_install.sh [options]

Options:
	--profile <csv>        Comma-separated profiles (default: base,python,db)
...
if is_wsl; then
	die "Detected WSL, not a Hyper-V Ubuntu guest. Use ./wsl_post_install.sh instead."
fi

if is_hyperv_guest; then
	log_info "Hyper-V guest detected."
else
	log_warn "Hyper-V was not detected. Continuing as a standard Ubuntu host."
fi
...
log_info "Completed Hyper-V Ubuntu bootstrap for profiles: ${PROFILES}"
